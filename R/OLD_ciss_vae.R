...
geti <- function(x, i) {
if (is_seq) {
# reticulate maps R's 1-based [[i]] to the Python item at i-1
# so use 1,2,3,... here.
...
# ---- sequentially peel off the tuple/list in the exact order Python returns ----
idx <- 1L
...
if (!(inherits(imputed_py, "python.builtin.object") &&
      (reticulate::py_has_attr(imputed_py, "to_numpy") ||
       reticulate::py_has_attr(imputed_py, "values")   ||
       reticulate::py_has_attr(imputed_py, "shape")))) {
  stop("run_cissvae(): Python did not return a dataframe-like object as first element.")
}
  ...
  # clusters (same behavior as before; if None we’ll set integer(0) or NULL—rare)
  if (isTRUE(return_clusters)) {
    out["clusters"] <- list(if (!is.null(clusters_py))
                              tryCatch(reticulate::py_to_r(clusters_py), error = function(e) NULL)
                            else NULL)
  }

  # silhouettes: if None, put NA_real_
  if (isTRUE(return_silhouettes)) {
    out["silhouettes"] <- list(if (!is.null(silh_py))
                                tryCatch(reticulate::py_to_r(silh_py), error = function(e) NA_real_)
                              else NA_real_)
  }

  # history: if None, keep named NULL (must assign via [ ] to preserve name)
  if (isTRUE(return_history)) {
    out["history"] <- list(if (!is.null(hist_py))
                            to_r_df_safely(hist_py)
                          else NULL)
  }
