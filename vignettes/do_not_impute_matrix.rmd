...
make_mock_survival_wide <- function(n_patients = 20,
                                    ...
                                    death_n = 3,
                                    mcar_pct = 0.10) {      # <- 10% random missingness
...
create_do_not_impute_matrix <- function(df,
                                        ...
                                        months = seq(0, 60, by = 6)) {
  # biomarker column names in expected order
  month_suffix <- sprintf("m%02d", months)
  biomarker_cols <- as.vector(outer(biomarker_prefixes, month_suffix, paste, sep = "_"))

  # subset only biomarker values
  bm <- as.matrix(df[, biomarker_cols, drop = FALSE])

  # initialize with zeros
  mask <- matrix(0L, nrow = nrow(bm), ncol = ncol(bm),
                 dimnames = list(df$patient_id, biomarker_cols))
  ...
  for (i in seq_len(nrow(df))) {
    ...
    if (!is.na(dy)) {
      cutoff_month <- dy * 12
      # which months are strictly after death
      post_idx <- which(months > cutoff_month)
      if (length(post_idx) > 0) {
        # all biomarker prefixes, so expand into col indices
...
To create the do_not_impute_matrix,


```{r}
dni  = create_do_not_impute_matrix(mock_df)
